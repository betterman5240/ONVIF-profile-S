...
int XmlGetStringValue(const char *str, const char *sName, char *sValue, int nLen)
{
	...
	strStart = (char *)strstr(str,strNameStart);
	if(strStart)
		strStart = strstr(strStart + 1,">");
	strEnd = (char *)strstr(str,strNameEnd);
...
{
	char *strPos = (char *)strstr(str,sValue);
	if(NULL == strPos)
		return 0;
	return 1;
}
...
int HttpParseHeader(struct Http_Buffer *pBuf,const char *key,const char *val)
{
	...
	else if(!soap_tag_cmp(key,"Content-Type"))
	{
		...
		{
			string strbuf=pBuf->Buffer;
			if(strbuf.find("<?xml")!=string::npos)
			{
				strncpy(pBuf->action,strbuf.substr(strbuf.find("<?xml"),strbuf.length()).c_str(),1024);
				pBuf->nHasAction = 1;
			}
		}
	}
