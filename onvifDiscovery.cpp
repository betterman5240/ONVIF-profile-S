...
const char * scopes_string="onvif://www.onvif.org/type/video_encoder onvif://www.onvif.org/type/audio_encoder onvif://www.onvif.org/Profile/Streaming onvif://www.onvif.org/hardware/Pi onvif://www.onvif.org/name/Pi onvif://www.onvif.org/location/";
...
int BuildNVTString(char *sBuffer,const char *suuid, struct Namespace *pNameSpace)
{
	...
	char struid[100];
	uuid_t uid;
	//GetLocalAddress(localIP,(char *)ETH_WIRE_DEV,(char *)"127.0.0.1");
	//hard code for IP.
	sprintf(localIP,"192.168.1.20");
	...
	strcat(sBuffer,TmpBuffer);
	sprintf(TmpBuffer,"<d:MetadataVersion>%d</d:MetadataVersion>",1);
	strcat(sBuffer,TmpBuffer);
...
void * ONVIF_DiscoveryThread(void * param)
{
	int sock;
	fd_set fset;
	int ret;
	...
	while (g_OnvifDiscoveryRunning)
	{
		...
		ret = recvfrom(sock, buffer, 10240 - 1, 0, (struct sockaddr*)&remote_addr, (socklen_t *)&len);
...
int ONVIF_DiscoveryStart()
{
	...
	ret = pthread_create(&g_OnvifServer.hDiscoveryThread, NULL,&ONVIF_DiscoveryThread, NULL);
	if (ret < 0)
	{
